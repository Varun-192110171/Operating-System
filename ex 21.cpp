...
void allocateMemory(struct MemoryBlock blocks[], int num_blocks, int process_id, int process_size) {
    ...
    for (int i = 0; i < num_blocks; i++) {
        if (!blocks[i].allocated && blocks[i].size >= process_size) {
            if (worst_fit_block_index == -1 || blocks[i].size > blocks[worst_fit_block_index].size) {
                worst_fit_block_index = i;
                max_free_space = blocks[i].size;
            }
        }
    }

    if (worst_fit_block_index != -1) {
        blocks[worst_fit_block_index].allocated = 1;
        printf("Process %d allocated to Block %d\n", process_id, blocks[worst_fit_block_index].block_id);
    } else {
